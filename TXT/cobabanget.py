...
transaction_history = [
    {'timestamp': '2024-01-01', 'quantity': 10},
    {'timestamp': '2024-01-02', 'quantity': 15},
    {'timestamp': '2024-01-03', 'quantity': 20},
]

def show_transaction_graph():
    if not transaction_history:
        messagebox.showwarning("Warning", "Tidak ada data transaksi untuk ditampilkan!")
        return
    
    # Persiapkan data
    dates = [transaction['timestamp'] for transaction in transaction_history]
    quantities = [transaction['quantity'] for transaction in transaction_history]
...
# UI
root = tk.Tk()
root.title("Aplikasi Transaksi")
tk.Button(root, text="Lihat Grafik Transaksi", command=show_transaction_graph).pack(pady=20)
root.mainloop()
...
class TransactionApp:
    def __init__(self):
        self.transaction_history = []  # Inisialisasi data transaksi
        self.create_ui()
    ...
    def create_ui(self):
        ...
        tk.Button(history_frame, text="Lihat Grafik Transaksi", command=self.show_transaction_graph).pack(pady=10)
    ...
    def show_transaction_graph(self):
        ...
        dates = [transaction['timestamp'] for transaction in self.transaction_history]
        quantities = [transaction['quantity'] for transaction in self.transaction_history]

        # Validasi dan urutkan data
        sorted_data = sorted(zip(dates, quantities), key=lambda x: x[0])
        sorted_dates, sorted_quantities = zip(*sorted_data)

        # Buat grafik
        plt.figure(figsize=(10, 6))
        plt.plot(sorted_dates, sorted_quantities, marker='o', linestyle='-', color='b')
        plt.title("Grafik Transaksi")
        plt.xlabel("Tanggal")
        plt.ylabel("Jumlah Transaksi")
        plt.xticks(rotation=45)
