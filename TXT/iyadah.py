    ...
    def simpan_edit():
        try:
            ...
            if 0 <= index < len(makanan_data):
                ...
                tampilkan_riwayat_makanan()
                window.destroy()
                simpan_data()  # Menyimpan data ke file setelah mengedit item
            else:
                messagebox.showerror("Error", "Index tidak valid!")
        except ValueError:
            messagebox.showerror("Error Input", "Index, Kategori, dan Warna harus berupa angka!")
...
# Buat tombol untuk menjalankan fungsi
add_button = tk.Button(root, text="Tambah Data Makanan", command=tambah_data)
add_button.pack(pady=10)

edit_button = tk.Button(root, text="Edit Data Makanan", command=edit_data)
edit_button.pack(pady=10)

delete_button = tk.Button(root, text="Hapus Data Makanan", command=hapus_data_makanan)
delete_button.pack(pady=10)

add_transaction_button = tk.Button(root, text="Tambah Transaksi", command=tambah_transaksi)
add_transaction_button.pack(pady=10)

view_food_button = tk.Button(root, text="Tampilkan Riwayat Makanan", command=tampilkan_riwayat_makanan)
view_food_button.pack(pady=10)

view_transaction_button = tk.Button(root, text="Tampilkan Riwayat Transaksi", command=tampilkan_riwayat_transaksi)
view_transaction_button.pack(pady=10)

riwayat_makanan_text = tk.Text(root, width=60, height=10)
riwayat_makanan_text.pack(pady=10)

riwayat_transaksi_text = tk.Text(root, width=60, height=10)
riwayat_transaksi_text.pack(pady=10)
